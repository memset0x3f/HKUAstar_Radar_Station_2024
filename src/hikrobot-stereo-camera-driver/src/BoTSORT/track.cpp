...
Track::Track(std::vector<float> tlwh, float score, uint8_t class_id,
    ...
    : det_tlwh(std::move(tlwh)), _score(score), class_id(class_id),
                        ...
                        uint32_t frame_id, bool new_id)
{
    ...
    _update_class_id(new_track.class_id, new_track._score);
    _update_tracklet_tlwh_inplace();
}
...
void Track::_update_class_id(uint8_t class_id_, float score)
{
    if (!_class_hist.empty())
    {
        // ROS_INFO("Num class: %d", num_cls);
        if(class_id_ >= num_cls) return;
        int max_freq = 0;
        bool found = false;

        for (auto &class_hist: _class_hist)
        {
            if (class_hist.first == class_id_)
            {
            ...
            if (static_cast<int>(class_hist.second) > max_freq)
            {
                ...
                class_id = class_hist.first;
            }
        }

        if (!found)
        {
            _class_hist.emplace_back(class_id_, score);
            class_id = class_id_;
        }
    }
    else
